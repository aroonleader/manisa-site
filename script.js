...
// ========== ربات پشتیبانی (نسخه ۲ — هوشمندتر) ==========
const chatQA = [
    { q: '🛍️ چطوری سفارش بدم؟', keys: ['سفارش', 'خرید', 'سبد', 'ثبت', 'بردارم', 'نحوه خرید'], a: 'خیلی ساده‌ست! 😊\n۱️⃣ محصول مورد نظرت رو به سبد خرید اضافه کن\n۲️⃣ دکمه «تایید و ارسال سفارش» رو بزن\n۳️⃣ سفارشت از طریق واتساپ ثبت می‌شه و ما راهنماییت می‌کنیم.' },
    { q: '🚚 هزینه و زمان ارسال', keys: ['ارسال', 'پست', 'تحویل', 'کی میاد', 'کی میرسه', 'پیک'], a: '🚚 برای خرید بالای ۲ میلیون تومان، ارسال کاملاً رایگانه!\n⏰ سفارش‌ها بین ۲ تا ۴ روز کاری به دستت می‌رسه.' },
    { q: '💳 نحوه پرداخت', keys: ['پرداخت', 'کارت', 'پول', 'واریز', 'درگاه'], a: '💳 بعد از ثبت سفارش در واتساپ، پرداخت به صورت کارت‌به‌کارت یا لینک پرداخت امن انجام می‌شه.\n🔒 داخل سایت هیچ پرداخت آنلاینی انجام نمی‌شه.' },
    { q: '🔄 شرایط مرجوعی', keys: ['مرجوع', 'برگشت', 'تعویض', 'پس بدم', 'راضی نبودم'], a: '🔄 تا ۷ روز بعد از دریافت کالا فرصت مرجوعی داری.\n✅ شرط: لباس استفاده نشده و اتیکت‌ها سالم باشن.' },
    { q: '📏 راهنمای سایز', keys: ['سایز', 'اندازه', 'قد', 'سایزم', 'چه سایزی'], a: '📏 سایزها از S تا XL هستن.\n💡 اگه بین دو سایزی، سایز بزرگ‌تر رو انتخاب کن.\nبرای راهنمایی دقیق‌تر، توی واتساپ پیام بده.' },
    { q: '👤 اتصال به پشتیبان انسانی', keys: ['پشتیبان', 'تماس', 'تلفن', 'واتساپ', 'انسان', 'ادمین'], a: 'HUMAN' }
];

const extraRules = [
    { keys: ['قیمت', 'چنده', 'چند است', 'گرون', 'ارزون', 'قیمتش'], a: '💰 قیمت هر محصول زیر همون محصول نوشته شده.\n🏷️ چند تا محصول هم الان تخفیف دارن — بج قرمزشون رو ببین!' },
    { keys: ['تخفیف', 'حراج', 'آف', 'کد تخفیف'], a: '🏷️ بله! الان چند محصول با ۱۰ تا ۲۰٪ تخفیف داریم.\nدنبال بج‌های قرمز روی محصولات بگرد! 😍' },
    { keys: ['موجود', 'ناموجود', 'انبار', 'دارید'], a: '📦 همه محصولاتی که توی سایت می‌بینی موجود هستن.\nاگه چیزی ناموجود بشه، از سایت حذف می‌شه.' },
    { keys: ['آدرس', 'کجاست', 'کجا', 'مغازه', 'حضوری', 'شعبه'], a: '🏬 ما فروشگاه آنلاین هستیم و به همه شهرها ارسال داریم.\nخرید حضوری فعلاً نداریم. 😊' },
    { keys: ['ساعت', 'کی جواب میدید', 'آنلاین', 'باز هستید'], a: '🕐 پشتیبانی واتساپ هر روز ۹ صبح تا ۹ شب پاسخگوست.\nمن (ربات) هم ۲۴ ساعته اینجام! 😄' },
    { keys: ['تشکر', 'مرسی', 'ممنون', 'عالی بود', 'thanks', 'thank'], a: 'خواهش می‌کنم! 🌹 اگه سوال دیگه‌ای داشتی در خدمتم.' },
    { keys: ['سلام', 'درود', 'hello', 'hi', 'صبح بخیر', 'عصر بخیر'], a: 'سلام! 😍 خوش اومدی به A&B.\nچطور می‌تونم کمکت کنم؟ از دکمه‌های پایین هم می‌تونی استفاده کنی. 👇' }
];

let chatOpened = false;

// تمیز کردن متن برای فهم بهتر
...
function findAnswer(text) {
    ...
    const all = [...chatQA, ...extraRules];
    for (const item of all) {
        for (const k of item.keys) {
            if (t.includes(normalizeText(k))) return item;
        }
    }
    return null;
}
...
function toggleChat() {
    ...
    if (!chatOpened && widget.classList.contains('open')) {
        chatOpened = true;
        setTimeout(() => {
            botSay('سلام! 👋 من ربات پشتیبانی A&B هستم. 🤖\nهر سوالی داری بنویس یا از دکمه‌های آماده پایین استفاده کن. 🎧');
            renderQuickButtons();
        }, 400);
    }
}
...
function sendChatMessage() {
    ...
    if (found) {
        ...
        botSay('🤖 هنوز دارم یاد می‌گیرم! 🙏\nمن به این موضوع‌ها جواب می‌دم: سفارش، ارسال، پرداخت، مرجوعی، سایز، قیمت، تخفیف، موجودی، آدرس و ساعت کاری.\nیا مستقیم توی واتساپ پیام بده. 💬');
    }
}
